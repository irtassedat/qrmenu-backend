...
router.get('/popular-products', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
  try {
    const { timeRange = 'week', branch_id, brand_id } = req.query;
    ...
    if (userRole === 'branch_manager') {
      ...
      // Brand managers can only see their brand data
      queryParams.push(userBrandId);
      brandFilter = ` AND b.brand_id = $${queryParams.length}`;
    } else if (userRole === 'super_admin') {
      // Super admin can filter by branch or brand if provided
      if (branch_id) {
...
router.get('/page-views', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
  try {
    const { timeRange = 'week', branch_id, brand_id } = req.query;
    const userRole = req.user.role;
    const userBranchId = req.user.branch_id;
    ...
    if (userRole === 'branch_manager') {
      queryParams.push(userBranchId);
      whereClause = ` AND e.branch_id = $${queryParams.length}`;
    } else if (userRole === 'brand_manager') {
      queryParams.push(userBrandId);
      whereClause = ` AND b.brand_id = $${queryParams.length}`;
    } else if (userRole === 'super_admin') {
      if (branch_id) {
        queryParams.push(branch_id);
        whereClause = ` AND e.branch_id = $${queryParams.length}`;
    ...
    const result = await db.query(`
      SELECT 
        TO_CHAR(DATE_TRUNC('day', e.created_at), '${format}') as date,
        COUNT(*) as views
      FROM events e
      LEFT JOIN branches b ON e.branch_id = b.id
      WHERE 
...
router.get('/clicks', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
  try {
...
router.get('/stats', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
  try {
...
router.get('/category-popularity', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
  try {
    const { timeRange = 'week' } = req.query;
