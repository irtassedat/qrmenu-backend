...
router.post('/branch/:id', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
    try {
        const { id } = req.params;
        const { integration_ids } = req.body;
        
        // Authorization check for branch managers
        if (req.user.role === 'branch_manager' && req.user.branch_id !== parseInt(id)) {
...
router.get('/branch/:id', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
    try {
        const { id } = req.params;
