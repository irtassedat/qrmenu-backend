...
// GET /api/orders/:id - Sipariş detaylarını getir (rol bazlı erişim)
router.get('/:id', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
    try {
        ...
        // Erişim kontrolü
        if (role === 'branch_manager' && order.branch_id !== userBranchId) {
            return res.status(403).json({ error: 'Bu siparişi görüntüleme yetkiniz yok' });
        }

        if (role === 'brand_manager' && order.brand_id !== userBrandId) {
            return res.status(403).json({ error: 'Bu siparişi görüntüleme yetkiniz yok' });
        }
...
// GET /api/orders - Tüm siparişleri listele (rol bazlı filtreleme)
router.get('/', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
    try {
        ...
        if (role === 'branch_manager') {
            ...
            query += ` AND b.brand_id = $${queryParams.length}`;
        }

        // Ek filtreler (sadece izin verilen kapsamda)
        if (branch_id) {
            // Branch filtrelemesi rol izinleriyle çakışmaz
...
// PATCH /api/orders/:id/status - Sipariş durumunu güncelle (rol bazlı erişim)
router.patch('/:id/status', authorize(['super_admin', 'brand_manager', 'branch_manager']), async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        const { role, brand_id: userBrandId, branch_id: userBranchId } = req.user;

        // Önce siparişin ve şubenin bilgilerini al
